...
class BaseLLMClient:
    """
    LLM客户端用于调用任何兼容OpenAI接口的服务, 并默认使用流式响应。
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not all([self.api_key, self.base_url]):
            raise BaseAgentsException("API密钥和服务地址必须被提供或在.env文件中定义。")
    ...
    def stream_invoke(self, messages: List[Message]) -> Iterator[str]:
        """
        调用大语言模型进行思考，并返回流式响应。
        ...
        Yields:
            str: 流式响应的文本片段
        """
        logger.info(f"stream invoke {self.model} model")
        if not isinstance(messages, list):
            raise BaseAgentsException("消息必须是列表类型")
        try:
            ...
            # 处理流式响应
            logger.info("start stream response")
            ...
            logger.info("call llm api raise err: {e}")
            raise BaseAgentsException(f"LLM调用失败: {str(e)}")
    ...
    def invoke(self, messages: List[Message]) -> str:
        ...
        适用于不需要流式输出的场景。
        """
        logger.info(f"invoke {self.model} model")
        if not isinstance(messages, list):
            raise BaseAgentsException("消息必须是列表类型")
        try:
            response = self._client.chat.completions.create(
